...
#import "TakeMemberOrderRstModel.h"
#import "QmemberSuccessController.h"
@interface QmemberConfirmController ()<UITableViewDataSource,UITableViewDelegate ,UITextFieldDelegate,UIAlertViewDelegate>
...
@end
@implementation QmemberConfirmController
...
- (void)viewDidLoad
{
    ...
    _qqMonth=[Utils trim:self.appDelegate.qqMonth];
    _qqMemberTypeName=[Utils trim:self.appDelegate.qqMemberTypeName];
    ...
    _titleView = [[TitleView alloc] initWithFrame:@"QQ会员" leftMenuItem:backBtn rightMenuItem:nil ];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0){
        ...
        if (indexPath.row == 2){
            ...
            [cell   setBackgroundColor:[UIColor whiteColor]];
            UILabel *label = [Utils labelWithFrame:CGRectMake(15.f, 5.f, screen_width-30, 21.f) withTitle:@"" titleFontSize:[UIFont systemFontOfSize:13.f] textColor:[UIColor blackColor] backgroundColor:[UIColor clearColor] alignment:NSTextAlignmentLeft];
            NSString *labelText=[NSString stringWithFormat:@"会员类型：%@", _qqMemberTypeName];
            NSMutableAttributedString *AttributedStr = [[NSMutableAttributedString alloc]initWithString:labelText ];
        ...
        if (indexPath.row == 3){
            ...
            [cell   setBackgroundColor:[UIColor whiteColor]];
            UILabel *label = [Utils labelWithFrame:CGRectMake(15.f, 5.f, screen_width-30, 21.f) withTitle:@"" titleFontSize:[UIFont systemFontOfSize:13.f] textColor:[UIColor blackColor] backgroundColor:[UIColor clearColor] alignment:NSTextAlignmentLeft];
            NSString *labelText=[NSString stringWithFormat:@"业务类型：%@", _qqMonth];
            NSMutableAttributedString *AttributedStr = [[NSMutableAttributedString alloc]initWithString:labelText ];
...
-(void) nextBtnClicked:(id)sender{
    ...
    [Utils alertTitle:@"确认充值" message:[NSString stringWithFormat:@"您确定对QQ号码为%@进行QQ服务办理?",_qqNumber] delegate:self cancelBtn:@"确认" otherBtnName:@"取消"];
}
...
-(void) takeOrder{
    ...
    [[NetworkSingleton sharedManager] takeMemberOrder:paraInfo   successBlock:^(id responseBody){
        
        TakeMemberOrderRstModel  *takeMemberOrderRstModel = [TakeMemberOrderRstModel objectWithKeyValues:responseBody];
        
        if(takeMemberOrderRstModel.respCode==0){
            
            [Utils setStoreValue:STORE_QQNUMBER storeValue:_qqNumber];
            [self gotoPay:takeMemberOrderRstModel.OrderId];
            
            
            
        }else{
            [_HUD hide:YES];
            [Utils alertTitle:@"提示" message:takeMemberOrderRstModel.respInfo delegate:self cancelBtn:@"确定" otherBtnName:nil];
...
-(void)gotoPay:(NSString *)orderId{
    ...
    [[NetworkSingleton sharedManager] qmemberPay:paraInfo   successBlock:^(id responseBody){
        
        [_HUD hide:YES];
        QmemberPayRstModel *qmemberPayRstModel = [QmemberPayRstModel objectWithKeyValues:responseBody];
        if(qmemberPayRstModel.respCode==0){
            ...
            QmemberSuccessController  *qmemberSuccessController=[[ QmemberSuccessController alloc]init];
            qmemberSuccessController.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:qmemberSuccessController animated:YES];
            
        }else{
            [Utils alertTitle:@"提示" message:qmemberPayRstModel.respInfo delegate:self cancelBtn:@"确定" otherBtnName:nil];
