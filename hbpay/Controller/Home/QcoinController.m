...
@interface QcoinController ()<UITableViewDataSource,UITableViewDelegate ,UITextFieldDelegate>
...
@end
@implementation QcoinController
...
- (void)viewDidLoad
{
    ...
    NSDictionary *paraInfo = [NSDictionary dictionaryWithObjectsAndKeys:[Utils getStoreValue:STORE_PHONENUMBER], @"PhoneNumber", QCOINBUSID, @"BusId", [Utils getStoreValue:STORE_QQNUMBER], @"TargetNumber",nil];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    ...
    if (section == 3){
        ...
        NSString *payTip=[Utils getStoreValue:STORE_QCOINPAYTIP];
...
- (void)nextBtnClicked:(id)sender{
    ...
    if([_productId isEqualToString:@""]){
        [Utils alertTitle:@"提示" message:@"请选择充值Q币个数" delegate:self cancelBtn:@"取消" otherBtnName:nil];
    ...
    QcoinConfirmController  *qcoinConfirmController=[[ QcoinConfirmController alloc]init];
    qcoinConfirmController.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:qcoinConfirmController animated:YES];
    return;
