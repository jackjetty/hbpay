#import "QcoinConfirmController.h"
...
#import "QcoinPayRstModel.h"
#import "TakeOrderRstModel.h"
#import "SMSCodeRstModel.h"
#import "Tool.h"
#import "LoginController.h"

@interface QcoinConfirmController ()<UITableViewDataSource,UITableViewDelegate ,UITextFieldDelegate,UIAlertViewDelegate>
...
@end
@implementation QcoinConfirmController
...
- (void)viewDidLoad
{
    ...
    _productId=[Utils trim:self.appDelegate.productId];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0){
        ...
        if (indexPath.row == 2){
            ...
            [cell   setBackgroundColor:[UIColor whiteColor]];
            UILabel *label = [Utils labelWithFrame:CGRectMake(15.f, 5.f, screen_width-30, 21.f) withTitle:@"" titleFontSize:[UIFont systemFontOfSize:13.f] textColor:[UIColor blackColor] backgroundColor:[UIColor clearColor] alignment:NSTextAlignmentLeft];
            NSString *labelText=[NSString stringWithFormat:@"Q币个数：%@",_qqCount];
            NSMutableAttributedString *AttributedStr = [[NSMutableAttributedString alloc]initWithString:labelText ];
    ...
    if (indexPath.section == 2){
        ...
        [cancelBtn setTitle:@"暂不支付" forState:UIControlStateNormal];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    if (section == 0) {
        ...
        UILabel *label = [Utils labelWithFrame:CGRectMake(15.f, 0.f, 300.f, 21.f) withTitle:@"" titleFontSize:[UIFont systemFontOfSize:12.f] textColor:RGB(149, 149, 149) backgroundColor:[UIColor clearColor] alignment:NSTextAlignmentLeft];
...
- (void)verifyBtnClicked:(id)sender{
    ...
    UIButton *btn = (UIButton *)sender; 
     ...
     NSDictionary *paraInfo = [NSDictionary dictionaryWithObjectsAndKeys:_phoneNumber, @"PhoneNumber", @"pay", @"MessageType",nil];
     ...
     //NSString *phoneNumberValue = [settings objectForKey:phoneKey];
        ...
        case Tag_VerifyTextField:
        {
            if ([textField text] != nil && [[textField text] length]!= 0) {
                ...
                if ([[textField text] length] != 6) {
                    ...
                    [Utils alertTitle:@"提示" message:@"短信验证码不等于6位！" delegate:nil cancelBtn:@"取消" otherBtnName:nil];
                }
            }
        }
            break;
...
-(void) nextBtnClicked:(id)sender{
    ...
    [Utils alertTitle:@"确认充值" message:[NSString stringWithFormat:@"您确定对QQ号码为%@进行充值?",_qqNumber] delegate:self cancelBtn:@"确认" otherBtnName:@"取消"];
...
-(void) takeOrder{
    ...
    NSDictionary *paraInfo = [NSDictionary dictionaryWithObjectsAndKeys:_phoneNumber, @"PhoneNumber", smsCode, @"SmsValiCode",
    ...
    [[NetworkSingleton sharedManager] takeOrder:paraInfo   successBlock:^(id responseBody){  
        //NSLog(@"rr %@",responseBody);
        TakeOrderRstModel *takeOrderRstModel = [TakeOrderRstModel objectWithKeyValues:responseBody];
        if(takeOrderRstModel.respCode==0){
            
            [Utils setStoreValue:STORE_QQNUMBER storeValue:_qqNumber];
            [self gotoPay:takeOrderRstModel.OrderId];
            
            
            
        }else{
            [_HUD hide:YES];
            [Utils alertTitle:@"提示" message:takeOrderRstModel.respInfo delegate:self cancelBtn:@"确定" otherBtnName:nil];
...
-(void)gotoPay:(NSString *)orderId{
    ...
    [[NetworkSingleton sharedManager] qcoinPay:paraInfo   successBlock:^(id responseBody){
        
        [_HUD hide:YES];
        
        QcoinPayRstModel *qcoinPayRstModel = [QcoinPayRstModel objectWithKeyValues:responseBody];
        if(qcoinPayRstModel.respCode==0){
            
            self.appDelegate.phoneNumber=_phoneNumber;
            self.appDelegate.qqNumber =_qqNumber;
            self.appDelegate.qqCount=_qqCount;
            self.appDelegate.payMoney=_payMoney;
            QcoinSuccessController  *qcoinSuccessController=[[ QcoinSuccessController alloc]init];
            qcoinSuccessController.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:qcoinSuccessController animated:YES];
            
            
            
            
        }else{
            [Utils alertTitle:@"提示" message:qcoinPayRstModel.respInfo delegate:self cancelBtn:@"确定" otherBtnName:nil];
