...
#import "Tool.h"
#import "QmonthHandleSuccessController.h"

@interface QmonthHandleConfirmController ()<UITableViewDataSource,UITableViewDelegate ,UITextFieldDelegate,UIAlertViewDelegate>
...
@end
@implementation QmonthHandleConfirmController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 2){
        ...
        [confirmBtn setTitle:@"办理包月" forState:UIControlStateNormal];
...
-(void) nextBtnClicked:(id)sender{
    ...
    [Utils alertTitle:@"确认包月" message:[NSString stringWithFormat:@"您确定对QQ号码为%@进行包月充值?",_qqNumber] delegate:self cancelBtn:@"确认" otherBtnName:@"取消"];
...
-(void) takeOrder{
    ...
    [[NetworkSingleton sharedManager] takeMonthOrder:paraInfo   successBlock:^(id responseBody){
       
        TakeMonthOrderRstModel *takeMonthOrderRstModel = [TakeMonthOrderRstModel objectWithKeyValues:responseBody];
        if(takeMonthOrderRstModel.respCode==0){
            
            [Utils setStoreValue:STORE_QQNUMBER storeValue:_qqNumber];
            [self gotoPay:takeMonthOrderRstModel.OrderId];
            
        }else{
            [_HUD hide:YES];
            [Utils alertTitle:@"提示" message:takeMonthOrderRstModel.respInfo delegate:self cancelBtn:@"确定" otherBtnName:nil];
...
-(void)gotoPay:(NSString *)orderId{
    ...
    [[NetworkSingleton sharedManager] qmonthPay:paraInfo   successBlock:^(id responseBody){
        
        [_HUD hide:YES];
        
        QmonthPayRstModel *qmonthPayRstModel = [QmonthPayRstModel objectWithKeyValues:responseBody];
        if(qmonthPayRstModel.respCode==0){
            ...
            QmonthHandleSuccessController  *qmonthHandleSuccessController=[[ QmonthHandleSuccessController alloc]init];
            qmonthHandleSuccessController.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:qmonthHandleSuccessController animated:YES];
            
            
            
            
        }else{
            [Utils alertTitle:@"提示" message:qmonthPayRstModel.respInfo delegate:self cancelBtn:@"确定" otherBtnName:nil];
